...
<AccordionGroup>
  <Accordion title="Initial margin requirement">
    You must post enough collateral relative to the position size you're opening. The required amount is set by the market's **initial margin ratio** — the minimum fraction of the notional value that must be covered by collateral before the position is created. If your collateral falls short of this threshold, the trade is rejected.
  ...
  <Accordion title="Leverage bounds">
    Every market has both a minimum and a maximum allowed leverage. Your requested leverage must fall within that range — too low (below the minimum) or too high (above the maximum) both result in rejection. Perpetra supports up to 50× leverage, but the limit for a specific market may be lower.
